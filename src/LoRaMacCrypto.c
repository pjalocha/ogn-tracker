...
void LoRaMacJoinDecrypt( const uint8_t *buffer, uint16_t size, const uint8_t *key, uint8_t *decBuffer )
{
    ...
    // therefore not necessarily 32-bit aligned.  Keep AES block accesses
    // aligned even on targets which fault on unaligned word loads/stores.
    ...
    // With a CFList the encrypted payload has a second 16-byte block.
    if( size >= 32 )
    {
