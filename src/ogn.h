...
*/
   ...
   { int32_t Alt = Altitude; if(Alt<0) Alt=0; else Alt=(Alt+5)/10;
     ...
     int32_t Lon = getFANETcordic(Longitude);                                    // Longitude: [0.0001/60deg] => [cordic]
                             // other, glider, tow, heli, chute, drop, hang, para, powered, jet, gyro, balloon, air, UAV, ground, static
     const uint8_t FNTtype[16] = { 0,     4,     5,   6,     1,     5,    2,   1,     5,      5,   6,    3,      5,   7,    0,     0  } ; // convert aircraft-type from OGN to FANET
