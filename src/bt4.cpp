...
#ifdef WITH_BT4_SPP                            // classic BT with ESP-IDF
...
{ esp_err_t Err=ESP_OK;
  ...
  // Serial.printf("esp_bt_controller_mem_release() => %d (%d)\n", Err, esp_bt_controller_get_status());
  esp_bt_controller_disable();
  esp_bt_controller_deinit();
  Err = esp_bt_controller_init(&BTconf); if(Err!=ESP_OK) // return Err;
  // Serial.printf("esp_bt_controller_init() => %d (%d)\n", Err, esp_bt_controller_get_status());
  // Format_String(CONS_UART_Write, "esp_bt_controller_init()\n");
  Err = esp_bt_controller_enable((esp_bt_mode_t)BTconf.mode); if(Err!=ESP_OK) return Err;   // mode must be same as in BTconf
  // Format_String(CONS_UART_Write, "esp_bt_controller_enable()\n");
  ...
  // Format_String(CONS_UART_Write, "esp_bluedroid_init()\n");
  Err = esp_bluedroid_enable(); if(Err!=ESP_OK) return Err;                                 // enable the BT stack
  // Format_String(CONS_UART_Write, "esp_bluedroid_enable()\n");
